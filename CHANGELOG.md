...
## v2.16.6D-SF01 — Ship Forge Playtest Vertical Slice (2026-09-01)

- Integrated M4NFROID SHIP FORGE v2.9.0 under `tools/ship-forge/`.
- Added the first real Ship Forge player asset at `assets/sprites/player/ship/bank.png`.
- Registered `player/ship/bank` in both canonical and embedded sprite manifests.
- Wired `runtime/Player.js` to select the 9 bank frames deterministically from local lateral velocity.
- Runtime dependency remains one-way: Ship Forge exports assets; game consumes only canonical sprite data.
- Acceptance gate: verify the generated ship visibly in live gameplay before any further Ship Forge expansion.
